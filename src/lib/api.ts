...
// MOCK DATA - Trap Brasileiro Corrigido
...
const mockSongs: Song[] = [
  { id: 'trap1', title: 'O Preço', artist: 'MC Cabelinho', artistId: 'art_trap1', album: 'LITTLE HAIR', albumId: 'alb_trap1', duration: '2:48', artworkUrl: 'https://placehold.co/300x300.png?text=O+Preco', lyrics: 'Se eu não existo, quem vai te fazer tão bem?\nSó eu sei o preço pra te ter também...' },
  ...
  { id: 'trap7', title: 'X1', artist: 'MC Cabelinho', artistId: 'art_trap1', album: 'LITTLE HAIR', albumId: 'alb_trap1', duration: '3:00', artworkUrl: 'https://placehold.co/300x300.png?text=X1', lyrics: 'Hoje eu tô à toa, fumando um balão\nCom meus amigos, contando milhão...' },
...
export async function getFeaturedAlbums(): Promise<Album[]> {
  ...
  // Ensure "Anos Luz (Single)" can appear if its song is featured, or just generally.
  // For this example, let's ensure the main albums are featured.
  const featuredAlbumIds = ['alb_trap1', 'alb_trap5', 'alb_trap2'];
  const featured = mockAlbums.filter(album => featuredAlbumIds.includes(album.id));
  return simulateFetch(featured.length > 0 ? featured : mockAlbums.slice(0, 3));
}
